...
import { listen, message, openDialog, getAppVersion, newWindow } from "../ipc";
    ...
    const un = listen("menu", async (e) => {
      ...
      switch (id) {
        ...
        case "file:new-window":
          // 原生菜单"新建窗口"：走 window-new IPC 创建新窗口
          newWindow();
          break;
        case "file:info":
          // 原生菜单"文件属性"：派发 DOM 事件由 App 组件展示 FileInfo 面板
          window.dispatchEvent(new CustomEvent("textora:show-file-info"));
          break;
        case "file:open":
          await s.openFile();
          break;
