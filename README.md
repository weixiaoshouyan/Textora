...
- ✅ **专用查看器**：CSV/TSV 表格（自动分隔符探测、表头吸顶）、JSON 树形（展开/折叠、类型着色）、PDF 内嵌预览

### 文件与工作区

- ✅ **文件树侧边栏**：新建 / 重命名 / 删除（移入回收站）/ 过滤，展开状态记忆
...
- ✅ **编码与换行**：UTF-8 / GBK（无 BOM 自动识别）/ Latin-1 / UTF-16 等编码检测与保存，LF / CRLF 切换
- ✅ **最近打开的文件**：欢迎页展示最近 10 个文件，点击即开
- ✅ **多标签**：拖拽排序、关闭确认、关闭其他/全部

### 代码编辑（Notepad++ 对标）

- ✅ 代码折叠（花括号/缩进）、括号匹配高亮、活动行高亮、缩进参考线
- ✅ 智能自动补全（单词 + 代码片段）、字符边缘线（可在设置中配置列数）、Ctrl+滚轮缩放（持久化）
- ✅ 书签系统（Ctrl+F2 标记 / F2 跳转，WYSIWYG 与源码模式均支持）
- ✅ 行操作：排序 / 去重 / 去空行 / 制表符转换
- ✅ 符号树（函数 / 类 / 方法导航）
- ⚠️ 宏录制回放（实验性）：源码模式可用，WYSIWYG 模式暂不支持
...
- **单元测试**：Vitest + jsdom，**320 个用例**覆盖核心逻辑（保存竞态、会话恢复、窗口关闭协议、速率限制、正则防护、路径安全、导出净化、编码检测、CSV/JSON 解析、搜索、宏、AI 工具确认等）
- **E2E**：Playwright 渲染层冒烟测试（`npm run test:e2e`）；**Electron 全链路**（真实主进程 + 文件系统，`npm run test:e2e:electron`，需先 `npm run build`）
